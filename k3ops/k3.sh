...
cmd_serve(){
  ...
  # launch.sh reads K3_RECORD from its environment and ssh does not carry it,
  # so forward it explicitly -- without this the caller's `K3_RECORD=... k3.sh
  # serve ...` is silently ignored. ssh reassembles the remote command into ONE
  # string that the remote shell re-splits, so an empty argument disappears and
  # every later argument shifts -- which silently made NAME the first server
  # flag. Hence the sentinel.
  #
  # K3_PROFILE is gone with the launcher's named profiles: routing flags are
  # passed as trailing args now, which ssh carries like any other argument.
  ssh -o ConnectTimeout=45 -o ServerAliveInterval=30 "$HOST" bash -s -- \
    "${K3_RECORD:-__unset__}" "$name" "$@" <<'EOS'
K3_RECORD=$1; shift
NAME=$1; shift
...
ENVP=""
[ -n "$K3_RECORD" ] && ENVP="K3_RECORD=$K3_RECORD "
