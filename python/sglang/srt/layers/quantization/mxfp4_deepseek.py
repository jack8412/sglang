...
_USE_OFFICIAL_SHUFFLE = envs.SGLANG_MXFP4_USE_OFFICIAL_SHUFFLE.get()
...
class DeepSeekMxfp4MoEMethod:
    ...
    def create_moe_runner(self, layer, moe_runner_config):
        ...
        # Mainline DSV4 always threads a non-None swiglu_limit through
        # MoeRunnerConfig (the fork's SGLANG_DSV4_2604_SUBMODE gate was
        # retired upstream); other wrapped models may pass None, which
        # skips the gemm1 clamp entirely.
    ...
    def apply(
        ...
        if getattr(layer, "_v4_marlin_path", False):
            ...
            # The fork also accepted its hash topk format here; mainline
            # HashTopK emits StandardTopKOutput (the separate hash format
            # was folded into the standard format upstream), so the
            # standard branch covers it.
        ...
        if getattr(layer, "_v4_tk_path", False):
            ...
            # extraction below, which happens after this dispatch). Mainline
            # HashTopK emits StandardTopKOutput (the fork's separate hash
            # format was folded into the standard format upstream), so the
            # standard branch covers it.
            from sglang.srt.layers.quantization.v4_triton_kernels_moe import (
            ...
            # On mainline the StandardDispatcher never maps topk ids to the
            # local range for the flashinfer_mxfp4 backend, so the default
            # (SKIP_DISPATCHER_MAPPING=True) leaves the already-global ids
            # untouched; False restores the fork-era local->global re-offset.
            ...
            # When swiglu_limit is None apply_v4_triton_kernels_moe skips the
            # clamp. Origin: sglang 本身.
        ...
        # See the triton_kernels-path note above: mainline dispatchers keep
        # ids global for this backend, so the re-offset is off by default.
        if not envs.SGLANG_OPT_MXFP4_SKIP_DISPATCHER_MAPPING.get():
            local_expert_offset = layer.moe_ep_rank * layer.num_local_experts
            topk_ids = torch.where(
                topk_ids >= 0,
...
def _mxfp4_predicate(layer, server_args):
    env = envs.SGLANG_V4_USE_TRITON_KERNELS.get()
    if env == "1":
        do_wrap = True
    elif env == "0":
        do_wrap = False
